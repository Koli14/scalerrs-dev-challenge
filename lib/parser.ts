 ...
 */
export function extractArticle(html: string, docId: string, opts: ExtractOptions = {}): ParsedArticle {
  ...
  $body.find("img").each((_, el) => {
    ...
    images.push({
      imgSrc,
      driveUrl: driveUrl ?? (driveFileId ? `https://drive.google.com/file/d/${driveFileId}/view` : null),
      driveFileId,
      alt,
      onDrive: Boolean(driveFileId),
      publiclyShared: null,
